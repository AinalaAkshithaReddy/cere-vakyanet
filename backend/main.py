...
def _get_upload_limit_bytes() -> int:
    """Resolve max upload size (in bytes) from env, defaulting to 64MB."""
    default_mb = 64
    env_value = os.getenv("MAX_UPLOAD_SIZE_MB") or os.getenv("UPLOAD_MAX_MB")
    try:
        limit_mb = int(env_value) if env_value else default_mb
        if limit_mb <= 0:
            raise ValueError
    except (ValueError, TypeError):
        logger_warning_msg = (
            f"Invalid MAX_UPLOAD_SIZE_MB '{env_value}', falling back to {default_mb}MB"
        )
        logging.getLogger(__name__).warning(logger_warning_msg)
        limit_mb = default_mb
    return limit_mb * 1024 * 1024

MAX_UPLOAD_SIZE_BYTES = _get_upload_limit_bytes()
...
logger.info(f"Max upload size: {MAX_UPLOAD_SIZE_BYTES // (1024 * 1024)} MB")
...
def clean_text(text: str) -> str:
    ...
    Removes repeated newlines, extra spaces, page numbers, headers/footers,
    and fixes hyphenated words that break across lines.
    """
    if not text:
        return ""
    
    # Remove hyphenated words that break across lines (e.g., "educa-\ntion" -> "education")
    text = re.sub(r'(\w+)-\s*\n\s*(\w+)', r'\1\2', text)
    
    # Remove page numbers (common patterns: "Page 1", "1", "1/10", etc.)
    text = re.sub(r'\b(?:Page\s+)?\d+\s*(?:/\s*\d+)?\b', '', text)
    
    # Remove common header/footer patterns (repeated text at start/end of lines)
    lines = text.split('\n')
    if len(lines) > 4:
        # Check first 2 and last 2 lines for repetition
        first_lines = [l.strip().lower() for l in lines[:2] if l.strip()]
        last_lines = [l.strip().lower() for l in lines[-2:] if l.strip()]
        # Remove if first and last lines are identical (likely header/footer)
        if first_lines and last_lines and first_lines[0] == last_lines[-1]:
            lines = lines[2:-2]
        text = '\n'.join(lines)
...
def chunk_text_by_sentence(text: str, max_chars: int = 3500) -> List[str]:
    """
    Split text into chunks by sentences, never breaking a sentence across chunks.
    Each chunk will be approximately max_chars characters, but will never exceed it.
    """
    ...
    if not complete_sentences or len(complete_sentences) == 1:
        # Fallback: split by double newlines (paragraphs)
        paragraphs = text.split('\n\n')
        complete_sentences = [p.strip() for p in paragraphs if p.strip()]
    
    # Build chunks by adding sentences until we approach max_chars
    chunks = []
    current_chunk = ""
    ...
    for sentence in complete_sentences:
        ...
        if potential_size > max_chars and current_chunk:
    ...
    # Safety check: if any chunk is still too large, split it by words (last resort)
    final_chunks = []
    for chunk in chunks:
        if len(chunk) > max_chars:
            ...
            word_chunk = ""
            for word in words:
                if len(word_chunk) + len(word) + 1 > max_chars and word_chunk:
...
def translate_chunk(chunk: str, target_code: str, source_code: str = "auto", max_retries: int = 3) -> str:
    """
    Translate a single chunk safely with retries.
    If auto-detection fails, falls back to English source.
    """
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"Translating chunk (attempt {attempt + 1}/{max_retries}, {len(chunk)} chars)")
            
            # Try with auto-detection first
            if attempt == 0 and source_code == "auto":
                translator = GoogleTranslator(source='auto', target=target_code)
            else:
                # Fallback to English if auto fails
            ...
            logger.info(f"Chunk translated successfully: '{chunk[:50]}...' -> '{translated[:50]}...'")
            return translated.strip()
            
        except Exception as chunk_error:
            error_msg = str(chunk_error)
            logger.warning(f"Chunk translation attempt {attempt + 1} failed: {error_msg}")
            
            if attempt < max_retries - 1:
                # Wait before retry
                wait_time = (attempt + 1) * 0.5
                logger.info(f"Retrying chunk after {wait_time} seconds...")
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"Failed to translate chunk after {max_retries} attempts: {error_msg}"
                )
...
def combine_translated_chunks(chunks: List[str]) -> str:
    """
    Combine translated chunks correctly, ensuring no missing or duplicated text.
    """
    if not chunks:
        return ""
    ...
    combined = " ".join(valid_chunks)
    
    # Clean up any double spaces that might have been introduced
    combined = re.sub(r'\s+', ' ', combined)
    
    # Restore paragraph breaks (if original had them)
    # This is a simple heuristic - in practice, you might want to preserve original structure
    combined = combined.strip()
    
    return combined

def parallel_translate_chunks(chunks: List[str], target_code: str, source_code: str = "auto", max_workers: int = 5) -> List[str]:
    """
    Translate chunks in parallel with a concurrency limit.
    Maintains chunk order in the returned list.
    """
    if not chunks:
        return []
    ...
    def process_chunk(index: int, chunk: str) -> str:
        # Reuse translate_chunk logic per chunk
        return translate_chunk(chunk, target_code, source_code)
...
def translate_text(text: str, target_language: str) -> Dict[str, any]:
    """
    Translate text to target language using the improved pipeline:
    clean_text → chunk_text_by_sentence → translate_chunk → combine_translated_chunks
    """
    try:
        # Validate input
        if not text or not text.strip():
        ...
        logger.info(f"Starting translation to {target_language} ({target_code})")
        
        # STEP 1: Clean and normalize the text
        logger.info(f"Step 1: Cleaning text (original length: {len(text)} chars)")
        cleaned_text = clean_text(text)
        logger.info(f"Text cleaned (cleaned length: {len(cleaned_text)} chars)")
        ...
        try:
            # Use a sample of cleaned text for language detection
        ...
        # If source and target are same, return original (cleaned)
        ...
        # STEP 3: Check if text needs chunking
        text_length = len(cleaned_text)
        needs_chunking = text_length > 3500
        
        if needs_chunking:
            logger.info(f"Step 2: Text is large ({text_length} chars), splitting into sentence-safe chunks")
            
            # Split into sentence-safe chunks
            chunks = chunk_text_by_sentence(cleaned_text, max_chars=3500)
            ...
            # STEP 4: Translate each chunk safely
            logger.info(f"Step 3: Translating {len(chunks)} chunks in parallel (max 5 at a time)")
            translated_chunks = parallel_translate_chunks(chunks, target_code, source_code, max_workers=5)
            
            # STEP 5: Combine translated chunks
            logger.info(f"Step 4: Combining {len(translated_chunks)} translated chunks")
            translated = combine_translated_chunks(translated_chunks)
            
            if not translated or not translated.strip():
                raise ValueError("All chunks failed to translate or combined result is empty")
            
            logger.info(f"✓ All chunks translated and combined. Total length: {len(translated)} chars")
            ...
            # For smaller texts, translate directly
            logger.info(f"Text is small ({text_length} chars), translating directly")
            translated = translate_chunk(cleaned_text, target_code, source_code, max_retries=3)
            
            if not translated or not translated.strip():
                raise ValueError("Translation returned empty result")
            
            logger.info(f"✓ Translation successful: '{cleaned_text[:30]}...' -> '{translated[:30]}...'")
...
if __name__ == "__main__":
    ...
    # Note: limit_max_request_size is only available via command line
    # For programmatic use, run: uvicorn main:app --limit-max-request-size <bytes>
    # Or use the start scripts which include this parameter
    uvicorn.run(
        app,
        host="0.0.0.0",
        port=8000,
